...
# 将地址 sp+8*a2 处的值 load到寄存器a1中
.macro LOAD a1, a2 
    ld \a1, \a2 * XLENB(sp)
.endm

#将寄存器a1的值store到 sp+8*a2中
.macro STORE a1, a2
    sd \a1 ,\a2 * XLENB(sp)
.endm

.macro SAVE_ALL
    # 原子操作交换寄存器 sp和sscratch
...
__alltraps:
    SAVE_ALL #汇编宏，保存上下文环境
    mv a0,sp #保存栈顶指针，a0保存函数输入的第一个参数
    jal rust_trap #调用rust_trap函数，该函数返回后，跳转到__trapret

    .globl __trapret

__trapret:
    RESTORE_ALL #恢复trap的上下文
    sret
